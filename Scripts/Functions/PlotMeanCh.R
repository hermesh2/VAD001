plotMeanCh <- function (response, factor1, factor2, error.bars = c("se", "sd", 
                                                                   "conf.int", "none"), level = 0.95, xlab = deparse(substitute(factor1)), 
                        ylab = paste("mean of", deparse(substitute(response))), legend.lab = deparse(substitute(factor2)), CEX.mean =2,
                        main = "", pch = 1:n.levs.2, lty = 1:n.levs.2, Xlim=NULL,Ylim= NULL ,cex.leg=1,
                        position.leg = "topright",
                        col = palette(),PLOTBOX=T, PLOTAXIS=T,PLOTLEGEND=T, ...) {
  #Based in Rcmdr::plotMeans
  # PLOTBOX plots the box around the picture
  # PLOTAXIS plots the axis around the picture
  # CEX.mean controla el tamaño de las bolas
  ...
  if (missing(factor2)) {
    ...
    levs <- levels(factor1)
    n.levs <- length(levs)
    plot(c(1, n.levs), yrange, type = "n", xlab = xlab, ylab = ylab, xlim=Xlim, ylim= Ylim,
         axes = FALSE, main = main, ...)
    points(1:n.levs, means, type = "b", pch = 16, cex = CEX.mean)
    if(PLOTBOX){
      box()  
    }
    if(PLOTAXIS){
    ...
    if (n.levs.2 > length(col)) 
      stop(sprintf(gettextRcmdr("Number of groups for factor2, %d, exceeds number of distinct colours, %d."), 
                   n.levs.2, length(col)))
    plot(c(1, n.levs.1 * 1.4), yrange, type = "n", xlab = xlab,xlim=Xlim,ylim= Ylim,
    ...
    for (i in 1:n.levs.2) {
      points(1:n.levs.1, means[, i], type = "b", pch = pch[i], 
             cex = CEX.mean, col = col[i], lty = lty[i])
      if (error.bars != "none") 
    ...
    #text("topright", "legend.lab", adj = c(0, -0.5))
    if(PLOTLEGEND){
      legend(position.leg, levs.2, pch = pch, col = col, bty="n",
             lty = lty, cex=cex.leg)  
    }
  }
  invisible(NULL)
